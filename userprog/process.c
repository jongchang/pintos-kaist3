...
/* Starts the first userland program, called "initd", loaded from FILE_NAME. The new thread may be scheduled (and may even exit)
 * before process_create_initd() returns. Returns the initd's thread id, or TID_ERROR if the thread cannot be created. Notice that THIS SHOULD BE CALLED ONCE.
 * FILE_NAME에서 로드된 "initd"라는 첫 번째 사용자랜드 프로그램을 시작합니다. 새 스레드를 예약할 수 있으며 종료할 수도 있습니다.
 * process_create_initd()가 반환되기 전에 initd의 스레드 ID를 반환하거나, 
 * 스레드를 생성할 수 없는 경우 TID_ERROR을 반환합니다. 이 경우 ONCE를 호출해야 합니다. */
tid_t
process_create_initd (const char *file_name) { 
	char *fn_copy; // start_process 함수를 수행 시 사용하는 인자 값
	tid_t tid;

	/* Make a copy of FILE_NAME. Otherwise there's a race between the caller and load().
	 * FILE_NAME의 복사본을 만듭니다. 그렇지 않으면 호출자와 load() 사이에 경합이 발생합니다. */
	...
	/* Create a new thread to execute FILE_NAME. FILE_NAME을(를) 실행할 새 스레드를 만듭니다. */
	// 실행할 쓰레드 생성(쓰레드 이름[문자열], 우선순위[31], 생성된 쓰레드가 실행할 함수를 가리키는 포인터, initd의 수행 시 사용하는 인자 값)
 ...
 * 현재 실행 컨텍스트를 f_name으로 전환합니다. 실패 시 -1을 반환합니다. */
int
process_exec (void *f_name) {
	char *file_name = f_name;
	bool success;

	/* We cannot use the intr_frame in the thread structure. This is because when current thread rescheduled, it stores the execution information to the member.
	 * 스레드 구조에서 intr_frame을 사용할 수 없습니다. 
	 * 현재 스레드가 일정을 변경하면 실행 정보가 멤버에게 저장되기 때문입니다. */
	struct intr_frame _if;
	_if.ds = _if.es = _if.ss = SEL_UDSEG;
	_if.cs = SEL_UCSEG;
	_if.eflags = FLAG_IF | FLAG_MBS;

	/* We first kill the current context - 우리는 우선 현재의 상황을 죽입니다. */
	process_cleanup ();

	/* And then load the binary - 그리고 이진법을 로드합니다 */
	// load - file_name의 프로그램을 메모리에 탑재
	success = load (file_name, &_if); //success = load (file_name, &_if_.eip, &if_.esp);

	/* If load failed, quit. - 로드가 실패한 경우 종료합니다. */
	if (!success)
	{
		palloc_free_page (file_name);
		return -1; // 실행중인 쓰레드 종료
	}

	// Argument Passing
	// hex_dump (_if.rsp, _if.rsp, USER_STACK - (uint64_t)_if.rsp, true);

	/* Start switched process. - 전환 프로세스를 시작합니다. */
	do_iret (&_if);
	NOT_REACHED ();
}
...
/* Loads an ELF executable from FILE_NAME into the current thread. Stores the executable's entry point into *RIP and its initial stack pointer into *RSP.
 * Returns true if successful, false otherwise.
 * FILE_NAME의 ELF 실행 파일을 현재 스레드에 로드합니다. 실행 파일의 진입점을 *RIP에 저장하고 초기 스택 포인터를 *RSP에 저장합니다.
...
load (const char *file_name, struct intr_frame *if_) {
	...
	/* Allocate and activate page directory. - 페이지 디렉토리 할당 및 활성화. */
	t->pml4 = pml4_create (); // 페이지 디렉토리 생성
	if (t->pml4 == NULL)
		goto done;
	process_activate (thread_current ()); // 페이지 테이블 활성화

	/* Open executable file. - 실행 파일을 엽니다. */
	...
	/* Read and verify executable header. - 실행 파일 헤더를 읽고 저장 */
	...
	/* Read program headers. - 프로그램 헤더를 읽어와 저장 */
			...
			case PT_LOAD:
				if (validate_segment (&phdr, file)) {
					...
					if (phdr.p_filesz > 0) {
						 ...
						 * 정상적인 부분입니다. 디스크에서 처음 부분을 읽고 나머지 부분은 제로입니다. */
						read_bytes = page_offset + phdr.p_filesz;
						zero_bytes = (ROUND_UP (page_offset + phdr.p_memsz, PGSIZE)
								- read_bytes);
					} else {
						/* Entirely zero. Don't read anything from disk.
						 * 0. 디스크에서 아무것도 읽지 마십시오. */
						read_bytes = 0;
						zero_bytes = ROUND_UP (page_offset + phdr.p_memsz, PGSIZE);
					}
					// 배치정보로 파일을 메모리에 탑재
